...
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │ SavingLogic  │  │VaultManager  │  │DepositCert   │  │
│  │ (Business)   │  │(Fund Mgmt)   │  │(NFT)         │  │
│  └──────┬───────┘  └──────┬───────┘  └──────────────┘  │
│         │                  │                             │
│  ┌──────▼───────┐  ┌──────▼───────┐                    │
...
- SVG certificate creation
- ERC-721 standard compliance
- Etherscan/OpenSea integration
...
|--------|-----------|------------|--------|
| **Reentrancy Attack** | 🔴 High | ReentrancyGuard modifier | ✅ Implemented |
| **Unauthorized Withdrawal** | 🔴 High | ownerOf() verification | ✅ Implemented |
| **APR Manipulation** | 🟡 Medium | Snapshot at deposit | ✅ Implemented |
| **Vault Drain** | 🔴 High | Access control | ✅ Implemented |
| **Integer Overflow** | 🟡 Medium | Solidity 0.8+ | ✅ Built-in |
| **Emergency Exploit** | 🟡 Medium | Pausable pattern | ✅ Implemented |
| **Front-running** | 🟢 Low | No price oracle | N/A |
| **Flash Loan Attack** | 🟢 Low | Time-locked deposits | N/A |
...
```
Frontend Display:
User → NFT Gallery → Generate SVG (client-side) → Display

Etherscan Display:
Etherscan → contract.tokenURI(8) → Metadata API → Generate SVG → Return JSON
```
